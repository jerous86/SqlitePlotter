// Source: https://raw.githubusercontent.com/mackyle/sqlite/master/ext/misc/percentile.c
...
static void percentStep(sqlite3_context *pCtx, int argc, sqlite3_value **argv){
    ...
    const double rPct = sqlite3_value_double(argv[1]);
    ...
    Percentile *p = (Percentile*)sqlite3_aggregate_context(pCtx, sizeof(*p));
    if( p==0 ) return;
    ...
    ** Requirement 4 */
    ...
    if( isInfinity(y) ){ sqlite3_result_error(pCtx, "Inf input to percentile()", -1); return; }
...
static void percentFinal(sqlite3_context *pCtx){
    Percentile *p = (Percentile*)sqlite3_aggregate_context(pCtx, 0);
    if( p==0 ) return;
    if( p->a==0 ) return;
    if( p->nUsed ){
        qsort(p->a, p->nUsed, sizeof(double), doubleCmp);
        const double ix = (p->rPct-1.0)*(p->nUsed-1)*0.01;
        ...
        const double vx = v1 + (v2-v1)*(ix-i1);
...
int sqlite3_percentile_init( sqlite3 *db ){
    ...
    rc = sqlite3_create_function(db, "percentile", 2, SQLITE_UTF8, (void *) 0, (void *) 0, percentStep, percentFinal);
    return rc;
}
